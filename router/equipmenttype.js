...
router.delete('/:equipmentTypeId', async function(req, res) {
    try {
        ...
        await EquipmentType.findByIdAndRemove(req.params.equipmentTypeId);

        res.send(`El Tipo de equipo ${typeName} fue eliminado correctamente`);
    } catch (error) {
        console.error(error);
        res.status(500).send('Ha ocurrido un error al eliminar el tipo de equipo');
    }
});
