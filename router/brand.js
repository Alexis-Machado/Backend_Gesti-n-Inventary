...
router.delete('/:brandId', async function(req, res) {
    try {
        ...
        await Brand.findByIdAndRemove(req.params.brandId);

        res.send(`La Marca ${brandName} fue eliminada correctamente`);
    } catch (error) {
        console.error(error);
        res.status(500).send('Ha ocurrido un error al eliminar la marca');
    }
});
