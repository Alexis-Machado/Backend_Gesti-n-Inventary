...
router.delete('/:equipmentStatusId', async function(req, res) {
    try {
        ...
        const statusName = equipmentStatus.name; 
        await EquipmentStatus.findByIdAndRemove(req.params.equipmentStatusId);

        res.send(`El Estado de equipo ${statusName} fue eliminado correctamente`);
    } catch (error) {
        console.error(error);
        res.status(500).send('Ha ocurrido un error al eliminar el estado de equipo');
    }
});
